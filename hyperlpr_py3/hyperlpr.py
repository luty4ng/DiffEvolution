...
class LPR():
    ...
    def detectPlateRough(self,image_gray,resize_h = 720,en_scale =1.08 ,top_bottom_padding_rate = 0.05):
        ...
        image_gray = cv2.equalizeHist(cv2.cvtColor(image_color_cropped,cv2.COLOR_RGB2GRAY))
    ...
    def finemappingVertical(self,image,rect):
        ...
        rect[0]+=res[0]
        image = image[:,H:T+2]
        image_resized = cv2.resize(image, (int(136), int(36)))
    ...
    def SimpleRecognizePlateByE2E(self,image):
        ...
        return res_set, image_rgb
    
    def getPlateSliceByE2E(self,image):
        images = self.detectPlateRough(image,image.shape[0],top_bottom_padding_rate=0.1)
        for j,plate in enumerate(images):
            plate, rect  =plate
            image_rgb, rect_refine, ori_image = self.finemappingVertical(plate,rect)
        # cv2.imwrite('./images_out/image_rgb.png', image_rgb)
        # cv2.imwrite('./images_out/ori_image.png', ori_image)
        return image_rgb
