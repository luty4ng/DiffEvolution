...
def DetectPlate(image):
    ...
    for j,plate in enumerate(images):
        ...
        image_rgb = fv.finemappingVertical(image_rgb)
        cache.verticalMappingToFolder(image_rgb)
        return rect, image_rgb

def RecognizePlate(rect, image_rgb):
    res_set = []
    print("e2e:", e2e.recognizeOne(image_rgb))
    image_gray = cv2.cvtColor(image_rgb,cv2.COLOR_RGB2GRAY)
...
def RecognizePlateByE2E(rect, image_rgb):
    res_set = []
    res,confidence = e2e.recognizeOne(image_rgb)
    print(res,confidence)
    res_set.append([[],res,confidence])
...
def PreProcessByE2E(image):
    rect, image_rgb = DetectPlate(image)
    x_tempx = image_rgb
...
def GetModelByE2E():
    return e2e.pred_model

def DecodePredict(y_pred):
    y_pred = y_pred[:,2:,:]
    results = ""
    confidence = 0.0
    print(len(chars))
